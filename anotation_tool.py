...
def chop_tweet(tweet):
    ...
    if len(words) > 27:
        tweet = ''
        for i in range(0, len(words), 27):
            tweet += ' '.join(words[i:i+27]) + '\n'
    return tweet

def get_anotation_from_user(writer, line):
    # get user input for anotation
    # write to csv
    targets = ['race', 'nationality', 'religion', 'sex', 'ideology', 'disability', 'misogony']
    hate = input('hate? (y/n)')
